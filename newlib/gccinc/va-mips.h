/* PATCHED for the Xenosaga decomp: __va_next_addr in the eabi/mips64
   branch is rewritten from `((__AP += 8) - 8)` (advance-then-load, gcc
   folds the load to -8(ap)) to a statement expression that keeps the
   OLD pointer in a register and loads through it before advancing --
   `move v0,ap; lw x,0(v0); addiu ap,8`. That fetch-then-advance shape
   is what every varargs function in the original binary uses; the
   stock 2.9 header's shape never matches. Original header: EE-GCC
   2.9-ee-991111 include/va-mips.h. */
/* ---------------------------------------- */
...
#ifdef __mips64
#define __va_next_addr(__AP, __type)					\
  __extension__ ({ char *__va_old = (char *) (__AP);			\
		   (__AP) = (char *) (__AP) + __va_reg_size;		\
		   __va_old; })
#else
#define __va_next_addr(__AP, __type)					\
  (((__builtin_classify_type (* (__type *) 0) < __record_type_class	\
