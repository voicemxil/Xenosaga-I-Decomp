/*
 * eMessage: event/dialog message rendering. Ported from the SquareMan
 * xenosaga decomp (src/game/eMessage.c, same game/same ELF -- symbol
 * names and struct layouts carried over verbatim), adapted to this
 * project's conventions: data objects are extern (they live in the
 * asm data segments), int resolved to int.
 */

#include "common.h"

extern int xglFontGetSPcodeSize(int code, const char *str);
...
extern char moji[4096];
extern MessageSprite msg_spr[16];
...
/* eMessageMain: not yet decompiled (no draft in the SquareMan tree either) */
...
extern void eCursolSet(u8 *param_1, int param_2);
...
s32 eMessageNextPage(eMessage* Message, int param_2)
{
    if (Message->unk_1c)
    {
        if (param_2 == 0)
