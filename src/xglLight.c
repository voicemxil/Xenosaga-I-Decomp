...
void xglLightIntensityParallel(void *pLight, int nNo, void *pSrc)
{
    if (nNo < 3) {
        *(TI *)((char *)(pLight) + nNo * 0x20 + 0x10) = *(TI *)pSrc;
    }
}

/* Normalize and store one of a light's up-to-3 parallel directions */
void xglLightDirection(void *pLight, int nNo, void *pSrc)
{
    char *pDst = (char *)pLight + nNo * 0x20;
